...
class ExampleModule(core.module.Module):
    ...
    async def ping_command(self, args: list):
        if not args:
            return "Pong!"
        elif len(args) >= 1 and args[0] == "cookie":
...
# Core: Module Loading (`core.modules`)

The `core.modules` file provides the engine for OpenLumara's extensibility. It is responsible for dynamically discovering, importing, and identifying modules and channels.

## Dynamic Discovery

OpenLumara uses filesystem scanning to find modules. Users can simply drop a new `.py` file into the `modules/`, `user_modules/`, or `channels/` directory, and the system will automatically pick it up on the next restart.

The `load()` function performs the following steps:
1.  **Package Scanning**: Uses `pkgutil` to iterate through all sub-modules within a given package.
2.  **Dependency Check**: Before importing, parses the module file with `ast` to extract the `dependencies` list and checks if they're installed. Skips modules with missing deps (logs a warning unless `loading_config=True`).
3.  **Conditional Import**: Only imports modules that are present in the `filter` list.
4.  **Reload Support**: If `reload=True`, forces a reload via `importlib.reload()`.
5.  **Class Inspection**: Scans for classes inheriting from `base_class`.
6.  **Filtering**: Returns only valid, relevant classes. Skips the base class itself.
7.  **Error Handling**: Catches all exceptions. Failed modules are added to `reported_broken` to avoid repeated logging.

## Dependency Auto-Installer

OpenLumara automatically manages Python dependencies:

| Function | Description |
| :--- | :--- |
| `install_module_deps(package, module_name, manager)` | Checks if dependencies are installed. If not, runs `pip install --quiet`. |
| `uninstall_module_deps(package, module_name, manager, exclude)` | Uninstalls dependencies only if not required by enabled modules. Calls `on_uninstall()` hook before uninstalling. |

Dependencies are extracted using AST parsing (no import needed), looking for a `dependencies = [...]` class attribute.

## Naming Convention

OpenLumara automatically converts `CamelCase` class names to `snake_case`:
- Class: `LifeOrganizer` → Module Name: `life_organizer`
- Class: `TelegramChannel` → Channel Name: `telegram_channel`

## Key Functions

| Function | Description |
| :--- | :--- |
| `load(package, base_class=None, filter=None, reload=False, loading_config=False)` | Discovers and imports classes from a package. |
| `get_name(obj)` | Converts a class name or instance to `snake_case` identifier. |

## Logging

During early initialization, log messages are buffered in `core.modules.log_buffer` as tuples of `(category, message)`. Once the manager is available, `Manager._drain_log_buffers()` processes them.

## Module Globals

| Variable | Description |
| :--- | :--- |
| `nonagentic` | Tuple of modules whose prompts are injected even when tools are off: `("characters", "writing_style", "time")`. |
| `reported_missing` | List of module names skipped due to missing dependencies. |
| `reported_broken` | List of module names that failed to load. |
| `log_buffer` | List of `(category, message)` tuples buffered during initialization. |
